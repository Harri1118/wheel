    ...
    pub async fn start_configured_agents(self: &Arc<Self>) {
        ...
        let (on_startup, parked_with_work): (Vec<Uuid>, Vec<Uuid>) = {
            ...
            let agents: Vec<(Uuid, bool)> = board::list(&conn)
                ...
                .collect();
            // Parking it would erase its reset time and resume it straight
            // into the closed window -- excluded from BOTH lists below,
            // whichever one would otherwise have put it there.
            let not_rate_limited = |id: &Uuid| {
            ...
            let on_startup = agents
                .into_iter()
                .filter(|(id, on_startup)| *on_startup && not_rate_limited(id))
                .map(|(id, _)| id)
                .collect();
